...
//Changes:
//Added Units variable. in runDescription 20.02.2016
                     ...
                     int * num_of_hists)
{
    ...
    {
        fprintf( stderr, "MUD problem: error while reading file %s\n", fin_name );
        goto error;
    }
...
runDescription rundesc(char * fin_name[])
{
    ...
    {
        MUD_free(pMUD_fileGrp);
        fprintf( stderr, "MUD problem: Error while reading %s\n", fin_name );
        goto end;
    }
    fclose( fin );
    //---------------------------------------------------------------------------
    ...
    else
    {
        ...
        for (index = 0; index <nmax; index ++)
        {
            ...
            //MUD_getIndVarNameUnits(fh,index+1,units,strdim);
            //strcpy(DESC.Units[index],units);
        }
        MUD_closeRead(fh);
    }
